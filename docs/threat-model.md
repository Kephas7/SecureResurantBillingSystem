...
| Tables endpoint | Tampering | Deleting a table that still has historical order references, corrupting referential/audit integrity | FK constraint plus an explicit any-order-history check in `TablesService.remove` (found and fixed during Day 5 testing — see `docs/day5-orders-testing.md`) | Mitigated |
